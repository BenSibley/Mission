...
if ( $author_display == 'no' ) {
	...
	printf( esc_html_x( 'Published %s', 'This blog post was published on some date', 'ct-theme-name' ), esc_html( $date ) );
} elseif ( $date_display == 'no' ) {
	// translators: placeholder is the author who published the post
	printf( esc_html_x( 'Published by %s', 'This blog post was published by some author', 'ct-theme-name' ), esc_html( $author ) );
} else {
	// translators: placeholders are the date the post was published and the author who published it
	printf( esc_html_x( 'Published %1$s by %2$s', 'This blog post was published on some date by some author', 'ct-theme-name' ), esc_html( $date ), esc_html( $author ) );
}
echo '</div>';
